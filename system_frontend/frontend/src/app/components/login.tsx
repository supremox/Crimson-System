...
const Login = () => {
  ...
  const onSubmit = (data: React.FormEvent) => {
    data.preventDefault();
    axiosInstance.post("/api/token/", { email, password })
      .then((res) => {
  ...
  return (
    <>
      ...
      <div
        className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-100 to-pink-100"
        style={{ backgroundImage: "url('/img/background.jpg')" }}
      >
        <div className="flex w-full max-w-3xl bg-white rounded-2xl shadow-lg overflow-hidden">
          {/* Left Side */}
          <div className="w-full md:w-1/2 p-10">
            <h2 className="text-red-950 font-bold text-center text-xl mb-2">
              Crimson System
            </h2>
            <hr></hr>
          ...
          <div className="hidden md:flex w-1/2 items-center justify-center p-0">
            <div
              className="flex flex-1 h-full w-full rounded-l-3xl items-center justify-center bg-cover bg-center"
              style={{ backgroundImage: "url('/img/bgpic.jpg')" }}
            >
              <div className="relative flex bg-black opacity-70 items-center h-full w-full rounded-l-3xl justify-center">
                ...
                <img
                  ...
                  className="max-h-80 object-contain relative z-10 w-40 h-40"
                />
              </div>
            </div>
          </div>
